package com.mycompany.placecommenter.wxService;

import com.mycompany.placecommenter.wxService.*;
...
import com.commenterteam.commenter.service.PlaceServiceImpl;
import com.commenterteam.commenter.service.UserServiceImpl;
import com.mycompany.placecommenter.wxService.util.*;
			...
			if(msgType == 51){
				...
				if(SpecialUsers.contains(msg.getString("ToUserName"))){
					...
					LOGGER.info(name + ": " + content);
                                        ResolveName(msg.getString("FromUserName"));
                                         String placeName = ResolveContent(content);
					String ans = getPlaceUrl(msg.getString("ToUserName"),content);
...
//						break;
	...
	public static void main(String[] args) throws InterruptedException {
        ...
        private void ResolveName(String name) {
            ...
            if(userServiceImpl.findUserByAccount(name) == null) {
                User user = new User();
                user.setAccount(name);
                user.setName(getUserRemarkName(name));
        ...
        private String ResolveContent(String content) {
            if(content.contains("<br/>/")) {
                content = content.substring(0,content.indexOf("<br/>/") - 2);
            } 
            //解析经纬度
            //查找数据库是否存在直接返回content
            //数据库不存在,添加数据,返回content
            return content;
        }
	
	private String getPlaceUrl(String userName,String placeName) {
                PlaceServiceImpl placeServiceImpl = new PlaceServiceImpl();
                Place place = placeServiceImpl.getPlaceByName(placeName);
                UserServiceImpl userServiceImpl = new UserServiceImpl();
                User user = userServiceImpl.findUserByAccount(userName);
                Integer placeId = place.getId();
                Integer userId = user.getId();
		String result = "http:10.0.14.173:8080/place/" + placeId + "/user/" + userId;
		return result;
	}
